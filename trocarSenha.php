...
<?php
  ...
  if($senha1==$senha2){
    $stmt = $mysqli->prepare("SELECT senha FROM terapeuta WHERE id_terapeuta = '$_SESSION[ID]'"); // Preparar a consulta
    if ($stmt === false) {
    ...
    if ($result->num_rows > 0){
      ...
      $stmt_update = $mysqli->prepare("UPDATE terapeuta SET senha = '$nova_senha_hash' WHERE id_terapeuta = '$_SESSION[ID]'"); //preparar alteração
